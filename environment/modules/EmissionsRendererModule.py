...
class EmissionColors:
	...
	vunhealty = np.array([102/255, 0, 153/255, 1])
...
class EmissionsRendererModule(BaseModule):
	def __init__(self, emissions, emission_types = [], save_to_file=False):
		...
		self.max_values = {"CO":1000,"CO2":10000, "NOx": 17, "HC": 10, "PMx": 1}
		...
		for i, emission_type in enumerate(emission_types):
			...
			im = ax.imshow(emissions.get_emissions_type_matrix(emission_type), cmap=cmap, vmin=0, vmax=self.max_values[str(emission_type)])
	...
	def _CO_colorbar(self):
		...
		_colors[:int(20000/24), :]    = EmissionColors.vunhealty
	...
	def step(self, timestep):
		...
		for i,emission_type in enumerate(self.emission_types):
			ax = self.axs[i]
			self.imgs[i].set_data(self.emissions.get_emissions_type_matrix(emission_type))
