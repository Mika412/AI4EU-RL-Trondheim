...
def create_detections(map_location, sensors_directory, hours, percentage, offset):
    ...
    processSensorData(map_location + map_filename, sensors_directory + sensor_filename,
                      sensors_directory + sensor_data_dir, map_location + map_output_directory, max_steps=hours,
                      percentage_to_use=percentage, start_offset=offset)


def create_routes(map_location):
    print("Creating routes and flows")
    flowrouterString = "$SUMO_HOME/tools/detector/flowrouter.py -n {map} -d {detector} -f {detections} -o {" \
                       "routesOutput} -e {" \
                       "flowOutput} -i 60 -l --random"
    flowrouterCommand = flowrouterString.format(map=map_location + map_filename,
...
def create_polys(map_location, poly_max_width, poly_max_height, poly_width, poly_height):
    print("Creating polys")
    generate_poly.generatePoly(map_location + poly_filename, poly_max_width, poly_max_height, poly_width, poly_height)

    print("Finding edges that belong to each poly")
    edgeConvert = "$SUMO_HOME/tools/edgesInDistricts.py -n {map} -t {polyfile} -o {output}"
    edgeCommand = edgeConvert.format(map=map_location + map_filename,
    ...
    config['DEFAULT'] = {'cell_max_height': poly_max_height,
...
def validate_arguments(args):
    if args.poly and (not args.poly_max_width or not args.poly_max_height):
        raise argparse.ArgumentError('--poly, requires poly-max-width and poly-max-height to be set ')
...
def execute_args(args):
    if args.det:
        create_detections(args.map, args.sensors, args.timesteps, args.percentage, args.offset)
    ...
    if args.poly:
        print(args)
        create_polys(args.map, args.poly_max_width, args.poly_max_height, args.poly_width, args.poly_height)
...
parser.add_argument("-t", "--timesteps", help="For how many timesteps in hours to generate data.", default=1, type=int)
parser.add_argument("-pe", "--percentage", help="For how many hours to generate data.", default=1,
                    type=validate_percentage)

parser.add_argument("-of", "--offset", help="Initial offset.", default=0,
                    type=int)
...
parser.add_argument('-r', '--routes', help="Create routes and flows", action='store_true')
